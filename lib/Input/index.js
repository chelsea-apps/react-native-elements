...
const Input = React.forwardRef(({ name, control, error, errorTestID, errorColor, errorStyle, label, validation, onSubmitEditing, defaultValue, infoText, infoTextStyle, infoTextColor, infoTextTestID, prefix, prefixStyle, postfix, postfixStyle, icon, style, textEntryStyle, textEntryContainerStyle, labelStyle, labelTopPosition, labelSmallFontSize, labelBigFontSize, bgColor, textColor, borderColor, focusedBorderColor, labelColor, focusedLabelColor, selectionColor, testID, textEntryTestID, labelTestID, keyboardType, ...props }, ref) => {
    /**
    ...
    const getKeyboardType = () => {
        if (['numeric', 'number-pad'].includes(keyboardType !== null && keyboardType !== void 0 ? keyboardType : ''))
